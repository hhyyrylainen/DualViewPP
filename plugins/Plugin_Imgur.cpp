...
static const auto IUContentLink = std::regex(".*i.imgur.com\\/.+", std::regex::ECMAScript | std::regex::icase);
...
static const auto ImgurIDCapture = std::regex(R"(.*imgur.com\/a\/(\w+).*)", std::regex::ECMAScript | std::regex::icase);
...
public:
    ...
    [[nodiscard]] bool HasCanonicalURLFeature() const override
    {
        return false;
    }

    std::string ConvertToCanonicalURL(const std::string& url) override
    {
        return {};
    }

    bool IsUrlNotGallery(const ProcessableURL& url) override
    {
        return false;
    }

    bool ScanAgainIfNoImages(const ProcessableURL& url) override
    {
        return false;
    }
...
private:
    ...
    static void ScanHtml(const std::string& body, const std::string& url, ScanResult& result)
    {
                        ...
                        [&](const gq::Node* image)
                        {
                            ...
                            if (std::regex_match(std::begin(link), std::end(link), IUContentLink))
                            {
                                ...
                                result.AddContentLink(ScanFoundImage(
                                    ProcessableURL(Leviathan::StringOperations::CombineURL(url, link.to_string()),
                                        std::string(), url)));
                            }
                        });

                    node->Each(".post-image-container",
                        [&](const gq::Node* contentLink)
                        {
                            const auto linkid = contentLink->GetAttributeValue(boost::string_ref("id"));
                            ...
                            for (size_t i = 0; i < videos.GetNodeCount(); ++i)
                            {
                                ...
                                result.AddContentLink(
                                    ScanFoundImage(ProcessableURL(Leviathan::StringOperations::URLProtocol(url) +
    ...
    [[nodiscard]] static std::string GetAjaxURL(const std::string& url, const std::ssub_match& match)
    {
        return Leviathan::StringOperations::URLProtocol(url) + "://imgur.com/ajaxalbums/getimages/" + match.str() +
            "/hit.json";
    }
    ...
    ScanResult ScanSite(const SiteToScan& params) override
    {
        ...
        if (params.ContentType == "application/json")
        {
            ...
            try
            {
                ...
                if (j.find("data") != j.end())
                {
                    ...
                    LOG_INFO("Imgur downloader: expecting " + Convert::ToString(count) + " images");
                    ...
                    for (const auto& image : j["data"]["images"])
                    {
                        ...
                        // Create link and add //
                        auto builtUrl =
                            Leviathan::StringOperations::URLProtocol(params.URL.GetURL()) + "://i.imgur.com/";
                        builtUrl += name;
                        builtUrl += extension;

                        result.AddContentLink(
                            ScanFoundImage(ProcessableURL(builtUrl, std::string(), params.URL.GetURL())));
                    }
                }
        ...
        {
            ScanHtml(params.Body, params.URL.GetURL(), result);
        }
        else
        {
            LOG_ERROR(std::string("Imgur downloader got unknown content type: ") + params.ContentType.data());
        }
...
#pragma ide diagnostic ignored "cppcoreguidelines-pro-type-static-cast-downcast"
    ...
    {
        return {std::shared_ptr<IWebsiteScanner>(new ImgurScanner(),
            [=](IWebsiteScanner* obj)
            {
