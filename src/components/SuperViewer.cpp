...
SuperViewer::SuperViewer(_GtkDrawingArea* area, Glib::RefPtr<Gtk::Builder> builder,
    ...
    DisplayedResource(displayedResource), Events(events), ForceOnlyThumbnail(forcethumbnail)
{
    // Do setup stuff //
    _CommonCtor();
}
...
bool SuperViewer::on_draw(const Cairo::RefPtr<Cairo::Context>& cr)
{
    ...
    if(!DisplayedResource && !DisplayImage) {
        ...
        refStyleContext->render_background(cr, allocation.get_x(), allocation.get_y(),
    ...
    const bool shouldBeThumbnailMode =
        ForceOnlyThumbnail || (width <= SUPER_THUMBNAIL_WIDTH_THRESHOLD &&
    ...
    if(!IsImageReadyToShow()) {
        ...
        for(int i = 1; i <= LoadingLineCount; ++i) {
            ...
            // Going up //
            cr->move_to(
                width / 2 - 25 * (((float)i / MAX_LOADING_LINES) + 1), height / 2 + (10 * i));
...
void SuperViewer::_DrawCurrentImage(const Cairo::RefPtr<Cairo::Context>& cr) const
{
    ...
    const auto topLeft = CalculateImageRenderTopLeft(
        CachedDrawnImage->get_width(), CachedDrawnImage->get_height(), ImageZoom);
...
// ------------------------------------ //
SuperViewer::Point SuperViewer::CalculateImageRenderTopLeft(
    size_t width, size_t height, float zoomlevel) const
{
    const Point centerpoint(
        (get_width() / 2) + BaseOffset.X, (get_height() / 2) + BaseOffset.Y);
...
void SuperViewer::_AddRedrawTimer(int32_t ms)
{
    ...
    Glib::signal_timeout().connect(
        sigc::bind<int32_t>(sigc::mem_fun(*this, &SuperViewer::_OnTimerCheck), ms),
        CurrentTimer);
}
...
void SuperViewer::_AddUnloadTimer()
{
    ...
    // Unload timer //
    Glib::signal_timeout().connect(
        sigc::mem_fun(*this, &SuperViewer::_OnUnloadTimer), SUPER_UNLOAD_IMAGE_AFTER_MS);
}
...
bool SuperViewer::_OnScroll(GdkEventScroll* event)
{
    ...
    if(BaseOffset.X != 0 || BaseOffset.Y != 0) {
        ...
        Point topleft = CalculateImageRenderTopLeft(
            DisplayImage->GetWidth(), DisplayImage->GetHeight(), ImageZoom);

        Point imagesizerelative = Point((mousepos.X - topleft.X) / (float)twidth,
        ...
        Point newtopleft = CalculateImageRenderTopLeft(
            DisplayImage->GetWidth(), DisplayImage->GetHeight(), ImageZoom);

        Point newimagesizerelative = Point((mousepos.X - newtopleft.X) / (float)twidth,
        ...
        Point pixeldifference =
            Point((int)(difference.X * twidth), (int)(difference.Y * theight));

        BaseOffset.X -= pixeldifference.X;
        BaseOffset.Y -= pixeldifference.Y;

        // Verification code //
        Point verifyleft = CalculateImageRenderTopLeft(
            DisplayImage->GetWidth(), DisplayImage->GetHeight(), ImageZoom);

        Point verifyrelative = Point((mousepos.X - verifyleft.X) / (float)twidth,
