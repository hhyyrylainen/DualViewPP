...
void Settings::Save()
{
    ...
    data.AddNamedVariable(std::make_shared<NamedVariableList>(
        "SettingsVersion", new IntBlock(SETTINGS_VERSION)));

    data.AddNamedVariable(std::make_shared<NamedVariableList>(
        "SavedWithVersion", new StringBlock(std::string("DualView ") + DUALVIEW_VERSION)));

    // Collection / database settings //
    {
        auto collection = std::make_shared<ObjectFileObjectProper>(
            "Collection", "", std::vector<std::unique_ptr<std::string>>());
        ...
        collectionList->AddVariable(std::make_shared<NamedVariableList>(
            "DatabaseFolder", new StringBlock(DatabaseFolder)));

        collectionList->AddVariable(std::make_shared<NamedVariableList>(
            "PublicCollection", new StringBlock(PublicCollection)));
    ...
    // Image view settings //
    {
        auto images = std::make_shared<ObjectFileObjectProper>(
            "Images", "", std::vector<std::unique_ptr<std::string>>());

        auto imagesDelays = std::make_unique<ObjectFileListProper>("delays");

        imagesDelays->AddVariable(
            std::make_shared<NamedVariableList>("NextImage", new FloatBlock(NextImageDelay)));

        images->AddVariableList(std::move(imagesDelays));

        auto imagesPreload = std::make_unique<ObjectFileListProper>("pre-load");

        imagesPreload->AddVariable(std::make_shared<NamedVariableList>(
            "CollectionForward", new IntBlock(PreloadCollectionForward)));

        imagesPreload->AddVariable(std::make_shared<NamedVariableList>(
            "CollectionBackwards", new IntBlock(PreloadCollectionBackwards)));
    ...
    // Download settings //
    {
        auto downloads = std::make_shared<ObjectFileObjectProper>(
            "Downloads", "", std::vector<std::unique_ptr<std::string>>());

        auto downloadsDelays = std::make_unique<ObjectFileListProper>("curl");

        downloadsDelays->AddVariable(
            std::make_shared<NamedVariableList>("Debug", new BoolBlock(CurlDebug)));

        downloadsDelays->AddVariable(
            std::make_shared<NamedVariableList>("MaxRetries", new IntBlock(MaxDLRetries)));
    ...
    // Plugins
    {
        auto plugins = std::make_shared<ObjectFileObjectProper>(
            "Plugins", "", std::vector<std::unique_ptr<std::string>>());
        ...
        pluginsConfig->AddVariable(std::make_shared<NamedVariableList>(
            "PluginsFolder", new StringBlock(PluginFolder)));
    ...
    if(!ObjectFileProcessor::WriteObjectFile(
           data, SettingsFile, DualView::Get().GetLogger())) {
...
void Settings::_Load()
{
    ...
    int settingsVersion;
    Leviathan::ObjectFileProcessor::LoadValueFromNamedVars(
        file->GetVariables(), "SettingsVersion", settingsVersion, -1, log, "Settings: Load:");
    ...
    if(collection) {
        ...
        if(settings) {
            ...
            Leviathan::ObjectFileProcessor::LoadValueFromNamedVars(settings->GetVariables(),
                "DatabaseFolder", DatabaseFolder, DatabaseFolder, log, "Settings: Load:");

            Leviathan::ObjectFileProcessor::LoadValueFromNamedVars(settings->GetVariables(),
                "PublicCollection", PublicCollection, PublicCollection, log,
                "Settings: Load:");
    ...
    if(images) {
        ...
        if(delays) {
            ...
            Leviathan::ObjectFileProcessor::LoadValueFromNamedVars(delays->GetVariables(),
                "NextImage", NextImageDelay, NextImageDelay, log, "Settings: Load:");
        ...
        if(preload) {
            ...
            Leviathan::ObjectFileProcessor::LoadValueFromNamedVars(preload->GetVariables(),
                "CollectionForward", PreloadCollectionForward, PreloadCollectionForward, log,
                "Settings: Load:");
    ...
    if(downloads) {
        ...
        if(curl) {
            ...
            Leviathan::ObjectFileProcessor::LoadValueFromNamedVars(
                curl->GetVariables(), "Debug", CurlDebug, CurlDebug, log, "Settings: Load:");

            Leviathan::ObjectFileProcessor::LoadValueFromNamedVars(curl->GetVariables(),
                "MaxRetries", MaxDLRetries, MaxDLRetries, log, "Settings: Load:");
    ...
    if(plugins) {
        ...
        if(settings) {
            ...
            Leviathan::ObjectFileProcessor::LoadValueFromNamedVars(settings->GetVariables(),
                "PluginsFolder", PluginFolder, PluginFolder, log, "Settings: Load:");
