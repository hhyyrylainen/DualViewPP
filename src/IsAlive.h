...
constexpr bool WARN_IF_NOT_ALIVE = false;
...
#define INVOKE_CHECK_ALIVE_MARKER(x)                                       \
    {                                                                      \
        if(!IsAlive::IsStillAlive(x)) {                                    \
            if(WARN_IF_NOT_ALIVE)                                          \
                LOG_WARNING("Object no longer alive in Invoked function"); \
            return;                                                        \
        }                                                                  \
    }
...
#define INVOKE_FUNCTION_WITH_ALIVE_CHECK(funcname) \
    {                                              \
        auto alive = this->GetAliveMarker();       \
