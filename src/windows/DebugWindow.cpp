...
// ------------------------------------ //
DebugWindow::DebugWindow(_GtkWindow* window, Glib::RefPtr<Gtk::Builder> builder) : Gtk::Window(window)
{
    ...
    TestImageRead->signal_clicked().connect(sigc::mem_fun(*this, &DebugWindow::OnTestImageRead));

    Gtk::Button* TestInstanceCreation;

    BUILDER_GET_WIDGET(TestInstanceCreation);

    TestInstanceCreation->signal_clicked().connect(sigc::mem_fun(*this, &DebugWindow::OnTestInstanceCreation));
}
...
void DebugWindow::OnTestImageRead()
{
    ...
    LEVIATHAN_ASSERT(boost::filesystem::exists(filePath), "OnTestImageRead preset image is missing");

    int width, height;
    std::string extension;

    LEVIATHAN_ASSERT(
        CacheManager::GetImageSize(filePath, width, height, extension), "OnTestImageRead image size get failed");

    LEVIATHAN_ASSERT(CacheManager::GetImageSize("/home/hhyyrylainen/803085.png", width, height, extension),
...
void DebugWindow::OnTestInstanceCreation()
{
    ...
    auto img1 = InternetImage::Create(
        ScanFoundImage(ProcessableURL("http://test.com/img.jpg", std::string(), "dualview")), false);
}
