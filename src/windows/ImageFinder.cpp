    ...
    Gtk::Window(window)
{
    ...
    MainSearchBar->signal_changed().connect(
        sigc::mem_fun(*this, &ImageFinder::OnSearchChanged));
...
void ImageFinder::OnSearchChanged()
{
    ...
    DualView::Get().QueueDBThreadFunction([=]() {
        ...
        try {
            ...
            _OnFailSearch(std::string("Invalid tag: ") + e.what());

            return;
        }

        const auto dbTag = DualView::Get().GetDatabase().SelectExistingAppliedTagIDAG(*tags);
        ...
        loadedResources->insert(
            std::end(*loadedResources), std::begin(images), std::end(images));
        ...
        auto itemselect = std::make_shared<ItemSelectable>([=](ListItem& updateditem) {
            DualView::Get().InvokeFunction([=]() {
                ...
                if(count == 0) {
                    ...
                    SelectStatusLabel->set_text(
                        "Selected " + std::to_string(count) + " items");
                }
            });
        });
        ...
        DualView::Get().InvokeFunction([=]() {
            ...
            Container->SetShownItems(
                loadedResources->begin(), loadedResources->end(), itemselect);

            _SetSearchingState(false);

            // Update status label about how many things we have found //
            FoundImageCountLabel->set_text(
                "Found " + std::to_string(loadedResources->size()) + " images");
        });
    });
}
// ------------------------------------ //
