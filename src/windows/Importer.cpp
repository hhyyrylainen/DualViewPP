    ...
    Gtk::Window(window)
{
    ...
    BrowseForImages->signal_clicked().connect(
        sigc::mem_fun(*this, &Importer::_OnBrowseForImages));
    ...
    AddImagesFromFolder->signal_clicked().connect(
        sigc::mem_fun(*this, &Importer::_OnAddImagesFromFolder));
    ...
    ImageList->signal_drag_data_received().connect(
        sigc::mem_fun(*this, &Importer::_OnFileDropped));
    ...
    CopyToCollection->signal_clicked().connect(
        sigc::mem_fun(*this, &Importer::_OnCopyToCollection));
    ...
    MoveToCollection->signal_clicked().connect(
        sigc::mem_fun(*this, &Importer::_OnMoveToCollection));
    ...
    DeselectCurrentImage->signal_clicked().connect(
        sigc::mem_fun(*this, &Importer::_OnDeselectCurrent));
    ...
    RemoveSelectedButton->signal_clicked().connect(
        sigc::mem_fun(*this, &Importer::_RemoveSelected));
}
...
bool Importer::_AddImageToList(const std::string& file)
{
    ...
    for(const auto& image : ImagesToImport) {
        ...
        auto dialog = Gtk::MessageDialog(*this, "Add the same image again?", false,
            Gtk::MESSAGE_QUESTION, Gtk::BUTTONS_YES_NO, true);

        dialog.set_secondary_text(
            "Image at path: " + file + " has already been added to this importer.");
    ...
    for(auto iter = ImagesToImport.begin(); iter != ImagesToImport.end(); ++iter) {
        ...
        for(auto iter2 = ImagesToImport.begin(); iter2 != ImagesToImport.end(); ++iter2) {
            ...
            if(image2->GetHash() == image->GetHash() &&
                ...
                auto dialog = Gtk::MessageDialog(*this, "Remove Duplicate Images", false,
                    Gtk::MESSAGE_QUESTION, Gtk::BUTTONS_YES_NO, true);

                dialog.set_secondary_text(
                    "Images " + image->GetName() + " at: " + image->GetResourcePath() +
                    "\nand " + image2->GetName() + " at: " + image2->GetResourcePath() +
...
bool Importer::StartImporting(bool move)
{
    ...
    if(CollectionName->get_text().empty()) {
        ...
        auto dialog = Gtk::MessageDialog(*this, "Import to Uncategorized?", false,
            Gtk::MESSAGE_QUESTION, Gtk::BUTTONS_YES_NO, true);
    ...
    if(move) {
        ...
        for(auto iter = SelectedImages.begin(); iter != SelectedImages.end(); ++iter) {
            ...
            if(found != ImagesToImportOriginalPaths.end()) {
                ...
                auto dialog = Gtk::MessageDialog(*this, "Delete Existing File?", false,
                    Gtk::MESSAGE_QUESTION, Gtk::BUTTONS_YES_NO, true);

                dialog.set_secondary_text("File at: " + pathtodelete +
                                          " \nis already in the database with the name: " +
                                          (*iter)->GetName() + "\nDelete the file?");
    ...
    ImportThread =
        std::thread(&Importer::_RunImportThread, this, CollectionName->get_text(), move);
    ...
    if(success) {
        ...
        if(!targetfolder->IsRoot()) {
            ...
            DualView::Get().AddCollectionToFolder(
                targetfolder, DualView::Get().GetDatabase().SelectCollectionByNameAG(
        ...
        if(RemoveAfterAdding->get_active()) {
            ...
            ImagesToImport.erase(std::remove_if(ImagesToImport.begin(), ImagesToImport.end(),
                                     [this](auto& x) {
                                         return std::find(SelectedImages.begin(),
                                                    SelectedImages.end(),
                                                    x) != SelectedImages.end();
                                     }),
                ImagesToImport.end());
        ...
        auto dialog = Gtk::MessageDialog(*this, "Failed to import selected images", false,
