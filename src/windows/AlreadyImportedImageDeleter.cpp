...
void AlreadyImportedImageDeleter::_UpdateButtonState()
{
    if(StopProcessing) {
        ...
        AlreadyImportedStartStopButton->get_style_context()->remove_class(
            "destructive-action");
...
{
    AlreadyImportedStartStopButton->set_sensitive(
        !AlreadyImportedCheckPathChooser->get_filename().empty());
}
// ------------------------------------ //
void AlreadyImportedImageDeleter::_RunTaskThread()
{
    ...
    while(!StopProcessing) {
        ...
        try {
            ...
            if(existing) {
                LOG_INFO("Found already existing image (" + hash + ") at path: " +
                         currentPath + " path in collection: " + existing->GetResourcePath());
                ...
                if(deleteChecked) {
                    LOG_INFO(
                        "File at path already exists in collection, deleting: " + currentPath);
        ...
        if(items % REPORT_PROGRESS_EVERY_N_ITEMS == 0) {
            DualView::Get().InvokeFunction([=]() {
                ...
                AlreadyImportedFilesCheckedLabel->set_text(
                    std::to_string(TotalItemsProcessed) + " items processed " +
                    std::to_string(TotalItemsDeleted) + " existing items deleted " +
                    std::to_string(TotalItemsCopiedToRepairCollection) +
                    " items used for collection repair");
            });
        }
    }
}
