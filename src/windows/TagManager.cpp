...
// ------------------------------------ //
TagManager::TagManager(_GtkWindow* window, Glib::RefPtr<Gtk::Builder> builder) : Gtk::Window(window)
{
    ...
    TagSearch->signal_search_changed().connect(sigc::mem_fun(*this, &TagManager::UpdateTagSearch));
    ...
    FoundTags->signal_row_activated().connect(sigc::mem_fun(*this, &TagManager::_OnSelectTagToEdit));
    ...
    DualView::Get().QueueDBThreadFunction(
        [=]()
        {
            ...
            DualView::Get().InvokeFunction(
                [this, isalive, tags{std::move(tags)}]()
                {
...
void TagManager::CreateNewTag()
{
    ...
    if (categoryNumber < 0 || categoryNumber > static_cast<int>(TAG_CATEGORY::QUALITY_HELPFULL_LEVEL))
    {
    ...
    Leviathan::StringOperations::CutLines<std::string>(NewTagAliases->get_buffer()->get_text(), newaliases);

    std::vector<std::string> newimplies;
    Leviathan::StringOperations::CutLines<std::string>(NewTagImplies->get_buffer()->get_text(), newimplies);
    ...
    DualView::Get().QueueDBThreadFunction(
        [=]()
        {
            ...
            for (const auto& imply : newimplies)
            {
                ...
                if (!foundtag)
                {
                    LOG_ERROR("Failed to create new tag, because implied tag doesn't exist: " + imply);
                    return;
                }
            ...
            try
            {
                auto tag = DualView::Get().GetDatabase().InsertTag(name, description, category, isprivate);
...
void TagManager::_ReadEditedTagData()
{
    ...
    // Find right one //
    for (auto iter = TagTypeStore->children().begin(); iter != TagTypeStore->children().end(); ++iter)
    {
    ...
    EditTagAliases->get_buffer()->set_text(
        Leviathan::StringOperations::StitchTogether<std::string>(EditedTag->GetAliases(), "\n"));
    ...
    EditTagImplies->get_buffer()->set_text(
        Leviathan::StringOperations::StitchTogether<std::string>(implystrings, "\n"));
}
...
void TagManager::_ApplyTagEdit()
{
    ...
    Leviathan::StringOperations::CutLines<std::string>(EditTagAliases->get_buffer()->get_text(), editedaliases);

    Leviathan::StringOperations::CutLines<std::string>(EditTagImplies->get_buffer()->get_text(), editedimplies);
        ...
        DualView::Get().InvokeFunction(
            [=]()
            {
                ...
                auto dialog = Gtk::MessageDialog(
                    *this, "Failed to apply tag changes", false, Gtk::MESSAGE_ERROR, Gtk::BUTTONS_OK, true);

                dialog.set_secondary_text("Error applying changes to tag id:" + Convert::ToString(targettag->GetID()) +
                    " \"" + targettag->GetName() + "\" error: " + message);
    ...
    DualView::Get().QueueDBThreadFunction(
        [=]()
        {
            try
            {
                ...
                Leviathan::FindRemovedElements(targettag->GetAliases(), editedaliases, added, removed);
                ...
                Leviathan::FindRemovedElements(implystrings, editedimplies, added, removed);

                for (const auto& value : added)
                {
                    targettag->AddImpliedTag(DualView::Get().GetDatabase().SelectTagByNameOrAlias(value));
                }

                for (const auto& value : removed)
                {
                    targettag->RemoveImpliedTag(DualView::Get().GetDatabase().SelectTagByNameOrAlias(value));
                }
            }
            catch (const InvalidSQL& e)
            {
                LOG_ERROR("TagManager: tag update failed, sql error: " + std::string(e.what()));
                onfail(e.what());
                return;
            }
...
void TagManager::_OnSelectTagToEdit(const Gtk::TreeModel::Path& path, Gtk::TreeViewColumn* column)
{
