    ...
    Gtk::Window(window)
{
    ...
    TagSearch->signal_search_changed().connect(
        sigc::mem_fun(*this, &TagManager::UpdateTagSearch));
    ...
    FoundTags->signal_row_activated().connect(
        sigc::mem_fun(*this, &TagManager::_OnSelectTagToEdit));
...
void TagManager::CreateNewTag()
{
    ...
    if(categoryNumber < 0 ||
        categoryNumber > static_cast<int>(TAG_CATEGORY::QUALITY_HELPFULL_LEVEL)) {
    ...
    DualView::Get().QueueDBThreadFunction([=]() {
        ...
        for(const auto& imply : newimplies) {
            ...
            if(!foundtag) {
                ...
                LOG_ERROR(
                    "Failed to create new tag, because implied tag doesn't exist: " + imply);
                return;
            }
        ...
        try {
            ...
            auto tag = DualView::Get().GetDatabase().InsertTag(
...
void TagManager::_ReadEditedTagData()
{
    ...
    EditTagAliases->get_buffer()->set_text(
        Leviathan::StringOperations::StitchTogether<std::string>(
            EditedTag->GetAliases(), "\n"));
...
void TagManager::_ApplyTagEdit()
{
    ...
    auto onfail = [=](const std::string& message) {
        DualView::Get().InvokeFunction([=]() {
            ...
            auto dialog = Gtk::MessageDialog(*this, "Failed to apply tag changes", false,
                Gtk::MESSAGE_ERROR, Gtk::BUTTONS_OK, true);

            dialog.set_secondary_text(
                "Error applying changes to tag id:" + Convert::ToString(targettag->GetID()) +
    ...
    DualView::Get().QueueDBThreadFunction([=]() {
        try {
            ...
            Leviathan::FindRemovedElements(
                targettag->GetAliases(), editedaliases, added, removed);
...
void TagManager::_OnSelectTagToEdit(
